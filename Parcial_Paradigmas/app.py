...
from forms import LoginForm, RegistrarForm, ClienteForm, ProductoFrom, MyConsulta, CargarVentaForm
...
@app.route('/cargarventas', methods=['GET', 'POST'])
def cargarventas():
    if 'username' in session:
        formulario = CargarVentaForm()
        if formulario.validate_on_submit():
                with open('datos.csv', 'a+') as archivo:
                    archivo_csv = csv.writer(archivo)
                    registro = [formulario.codigo.data, formulario.cliente.data, formulario.producto.data, formulario.cantidad.data, formulario.precio.data]
                    archivo_csv.writerow(registro)
                flash('Archivos subidos correctamente')
                return redirect(url_for('menu'))
        return render_template('cargar_ventas.html', form = formulario)
